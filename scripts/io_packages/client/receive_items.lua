...
function read_check_number()
    ...
    check_number_item_address = gummiInventory - 0x4 + 0x77
    check_number = ReadInt(check_number_item_address)
    return check_number
end

function read_start_inventory_written_byte()
    start_inventory_written_address = gummiInventory - 0x4 + 0x7B
...
function write_slides()
    ...
    slides_picked_up_array_address = evidence + 0x3FF
...
function write_shared_ability(shared_ability_value)
    ...
    shared_abilities_address = sharedAbilities - 0x1
...
function write_sora_ability(ability_value)
    ...
    abilities_address = soraCurAbilities - 0x1
    local i = 1
...
function write_item(item_offset)
    ...
    stock_address = inventory - 0x1
    WriteByte(stock_address + item_offset, math.min(ReadByte(stock_address + item_offset) + 1, 99))
end
